    ...
    def __init__(self, device, name_tokenizer, name_model):
        ...
        try:
            if 'distilbert' in name_model:
                self.lm_model = AutoModelWithLMHead.from_pretrained('distilbert-base-uncased', return_dict=True)
            elif 'bert' in name_model:
                self.lm_model = AutoModelWithLMHead.from_pretrained('bert-base-uncased', return_dict=True)
...
class WrapExplainEnc(WrapExplain):
    ...
    def fill_pad_ref_baseline(self, batch, seq_len):
        ...
        rt_pads = torch.tensor([ref for _ in range(batch)], dtype=torch.long, device=self.device)
        return rt_pads
    ...
    def fill_ref_baseline_lm_mask_filling(self, input_tensor, batch_sz, seq_len):
        ...
        repeated_input[range(seq_len), range(seq_len)] = self.tokenizer.mask_token_id
        ...
        for t in range(seq_len):
            ...
            mask_fill_tokens = [self.tokenizer.decode([x]) for x in mask_fill_token_ids]
            logging.info('Time: {:2d} {:_<10}{:_>10}{:_>10}{:_>10}'.format(t, original_tok,
                                                                           mask_fill_tokens[0],
                                                                           mask_fill_tokens[1],
                                                                           mask_fill_tokens[2]))
        ref = indicies[:, 99].tolist()
        logging.info("New Sentence: {}".format(self.tokenizer.decode(ref)))
        rt_pads = torch.tensor([ref for _ in range(batch_sz)], dtype=torch.long, device=self.device)
        return rt_pads
...
class WrapDistillBERT(WrapExplainEnc):
    def __init__(self, name_tokenizer, name_model, device):
        ...
        self.tokenizer = DistilBertTokenizer.from_pretrained(name_tokenizer, truncation=True, max_length=100)
        self.model = DistilBertForSequenceClassification.from_pretrained(name_model, return_dict=True)
    ...
    def get_full_embeddings(self, input_ids):
        ...
        position_embeddings = self.model.distilbert.embeddings.position_embeddings(position_ids)
    ...
    def forward(self, inputs_embeds, **kwargs):
        model_output = self.model.forward(inputs_embeds=inputs_embeds, **kwargs)
...
class WrapBERT(WrapExplainEnc):
    def __init__(self, name_tokenizer, name_model, device):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(name_tokenizer, truncation=True, max_length=30)
        self.model = BertForSequenceClassification.from_pretrained(name_model, return_dict=True)
        self.model.to(device)
